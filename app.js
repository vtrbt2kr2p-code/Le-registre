...
const TRANSLATIONS = {
  fr: {
    ...
    onboardStep4Title: '4. Exportez en PDF', onboardStep4Body: "Depuis chaque devis ou facture, exportez un PDF prêt à envoyer, avec vos coordonnées d'entreprise.",
  ...
  en: {
    ...
    onboardStep4Title: '4. Export to PDF', onboardStep4Body: 'From any quote or invoice, export a ready-to-send PDF with your company details.',
...
function renderOnboarding(){
  const steps = [
    {icon:'🔒', title:t('onboardStep1Title'), body:t('onboardStep1Body')},
    {icon:'📝', title:t('onboardStep2Title'), body:t('onboardStep2Body')},
    {icon:'➜', title:t('onboardStep3Title'), body:t('onboardStep3Body')},
    {icon:'⬇', title:t('onboardStep4Title'), body:t('onboardStep4Body')},
  ];
  return `
  <div class="overlay" style="z-index:60;">
    <div class="modal" style="max-width:440px;" onclick="event.stopPropagation()">
      <div class="modal-body" style="padding-top:26px;">
        ...
        ${steps.map(s => `
          <div style="display:flex;gap:12px;align-items:flex-start;padding:10px 0;border-top:1px solid var(--rule);">
            <div style="font-size:1.25rem;line-height:1;">${s.icon}</div>
            <div>
